...
int main() {
    ...
    unsigned char *write_buffer = (unsigned char *) malloc(SECTOR_SIZE * sectors_per_block_global);

    int i = 0;

    printf("Size of unsigned int: %d\n", sizeof(unsigned int));

    for (i = 0; i < SECTOR_SIZE * sectors_per_block_global; i++) {
    ...
    initialize_block(&block1, sectors_per_block_global);
    block1->next = 25;
    printf("1. Bloco inicializado na memória: %d %d\n", block1->address, block1->next);
    
    initialize_block(&block2, sectors_per_block_global);
    block2->next = 100;
    ...
    writeBlock(block_number, sectors_per_block_global, block1);
    // lê o bloco escrito no disco para outro endereço na memória
    read_block(&block2, block_number, sectors_per_block_global);
    // imprime os valores do bloco lido
    printf("\nBloco lido da memoria: %d %d\n", block2->address, block2->next);
    assert(assert_blocks_are_equal(block1, block2, sectors_per_block_global));
